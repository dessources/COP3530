...
public class BinarySearchTree {
    ...
    public boolean contain(String x) {
        if (!isEmpty()) {
            int compareVal = cmp(x, root.getLeftChild());
            if (compareVal == 0) {
                return true;
            } else if (compareVal == 1)
                return contain(x, root.getLeftChild().getRightChild());
            else
                return contain(x, root.getLeftChild().getLeftChild());
        } else
            return false;
    }
    ...
    public String find(String x) {
        if (!isEmpty()) {
            int compareVal = cmp(x, root.getLeftChild());
            if (compareVal == 0) {
                return root.getLeftChild().getInfo().meaning;
            } else if (compareVal == 1)
                return find(x, root.getLeftChild().getRightChild());
            else
                return find(x, root.getLeftChild().getLeftChild());
        } else
            return "";
    }
    ...
    public boolean replace(String x, String y) {
        if (!isEmpty()) {
            int compareVal = cmp(x, root.getLeftChild());
            if (compareVal == 0) {
                ElementType info = root.getLeftChild().getInfo();
                info.meaning = y;
                root.getLeftChild().setInfo(info);
                return true;
            } else if (compareVal == 1)
                return replace(x, y, root.getLeftChild().getRightChild());
            else
                return replace(x, y, root.getLeftChild().getLeftChild());
        } else
            return false;
    }
    ...
    private int cmp(String x, Node p) {
        if (x.compareTo(p.getInfo().word) < 0)
            return -1;
        else if (x.compareTo(p.getInfo().word) == 0)
            return 0;
        else
            return 1;
    }
